...
export default async function Home() {
  let showcaseClients: Array<{ id: string; name: string; company: string | null; websiteUrl: string | null; logoUrl: string | null }>;
  try {
    showcaseClients = await prisma.client.findMany({
      where: { showOnWebsite: true, websiteUrl: { not: null } },
      orderBy: { name: "asc" },
      select: { id: true, name: true, company: true, websiteUrl: true, logoUrl: true },
    });
  } catch {
    ...
    const fallback = await prisma.client.findMany({
      ...
      orderBy: { name: "asc" },
      select: { id: true, name: true, company: true, websiteUrl: true },
    });
