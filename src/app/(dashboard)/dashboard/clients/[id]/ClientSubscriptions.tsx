...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    paused: "bg-amber-500/12 text-amber-700 dark:bg-amber-500/25 dark:text-amber-300",
    cancelled: "bg-red-500/12 text-red-700 dark:bg-red-500/25 dark:text-red-300",
    expired: "bg-[#8A8F98]/20 text-[#8A8F98] dark:bg-white/15 dark:text-gray-400",
  };
  return (
        ...
        {subscriptions.map((sub) => (
          ...
          >
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <p className="font-semibold text-[#0B132B] dark:text-gray-100">{sub.service.name}</p>
