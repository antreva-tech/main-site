  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* Card summary */}
        {lead.company && (
          <p className="text-sm text-gray-600 dark:text-gray-400 truncate">{lead.company}</p>
        )}
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
          {lead.email && (
            <>
              <span className="text-gray-500 dark:text-gray-400">{t.dashboard.pipeline.email}</span>
              <a
                href={`mailto:${lead.email}`}
                className="text-[#1C6ED5] hover:underline truncate block"
              >
                {lead.email}
              </a>
            </>
          ...
          {lead.phone && (
            <>
              <span className="text-gray-500 dark:text-gray-400">{t.dashboard.pipeline.phone}</span>
              <a
                href={`tel:${lead.phone}`}
                className="text-[#1C6ED5] hover:underline truncate block"
              >
          ...
          <span className="text-gray-500 dark:text-gray-400">{t.dashboard.pipeline.source}</span>
          <span className={`${lead.source === "other" && lead.sourceOther ? "" : "capitalize"} text-gray-700 dark:text-gray-300`}>
          ...
          {lead.lineOfBusiness && (
            <>
              <span className="text-gray-500 dark:text-gray-400">{t.dashboard.common.lineOfBusiness}</span>
          ...
          <span className="text-gray-500 dark:text-gray-400">{t.dashboard.pipeline.stage}</span>
          <span className="sm:col-span-1" ref={dropdownRef}>
            {lead.stage === "won" ? (
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`h-2 w-2 flex-shrink-0 rounded-full ${STAGE_COLORS.won}`} />
                <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{t.dashboard.pipeline.stages.won}</span>
              ...
              <div className="relative mt-0.5">
                ...
                {dropdownOpen && (
                  <ul
                    role="listbox"
                    className="absolute left-0 right-0 top-full z-50 mt-1 max-h-56 overflow-auto rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 py-1 shadow-lg"
                    aria-label={t.dashboard.pipeline.stage}
                  >
          ...
          {lead.expectedValue != null && lead.expectedValue > 0 && (
            <>
              <span className="text-gray-500 dark:text-gray-400">{t.dashboard.pipeline.estimatedValue}</span>
              <span className="font-medium text-[#1C6ED5]">
        ...
        {lead.notes && (
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase mb-0.5">{t.dashboard.pipeline.notes}</p>
            <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap line-clamp-3">{lead.notes}</p>
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200 dark:border-gray-600">
          ...
          {waUrl && (
            <a
              ...
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border-2 border-[#25D366] text-[#25D366] dark:border-[#34D399] dark:text-[#34D399] text-sm font-medium hover:bg-[#25D366]/10 dark:hover:bg-[#25D366]/15 transition"
            >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-[#1C6ED5] text-white text-sm font-medium hover:bg-[#1557B0] dark:hover:bg-[#1C6ED5]/90 transition"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
