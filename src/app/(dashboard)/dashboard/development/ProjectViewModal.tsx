  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
          ...
          <div ref={dropdownRef} className="relative mt-0.5">
            <button
              ...
              className="w-full min-w-0 flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-2 text-left text-sm text-gray-900 shadow-sm transition hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-[#1C6ED5] focus:ring-offset-0 disabled:opacity-50"
            >
                ...
                {STAGE_KEYS.map((key, i) => (
                  <li
                    ...
                    className={`flex cursor-pointer items-center gap-2 px-3 py-2 text-sm transition ${
                      project.stage === key
                        ? "bg-[#1C6ED5]/10 text-[#0B132B] font-medium"
                        : highlightedIndex === i
        ...
        <div>
          <p className="text-xs text-gray-500 uppercase mb-2">Recent activity (last 3)</p>
            ...
            <>
              <ul className="space-y-2">
                {project.recentLogs.map((log) => (
                  <li
                    key={log.id}
                    className="p-2.5 bg-gray-50 rounded-lg border border-gray-100 text-sm"
                  >
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-200">
          <Link
            ...
            className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-[#1C6ED5] text-white text-sm font-medium hover:bg-[#1559B3] transition"
          >
            View full project
          </Link>
          <Link
